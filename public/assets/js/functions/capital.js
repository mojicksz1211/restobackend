		...
		$.ajax({
			...
			success: function (data) {
				...
				data.forEach(function (row) {
					...
					var status = '';
					if (row.ACTIVE == 1) {
						status = '<span class="badge bg-info" style="font-size:8px !important;">ACTIVE</span>';
					} else {
						status = '<span class="badge bg-danger" style="font-size:8px" !important;>INACTIVE</span>';
					}

					var btn = `<div class="btn-group">
            <button type="button" onclick="edit_capital(${row.IDNo}, '${row.FULLNAME}', '${row.AMOUNT}', '${row.REMARKS}' )" class="btn btn-sm btn-alt-secondary js-bs-tooltip-enabled"
            ...
            <button type="button" onclick="archive_capital(${row.IDNo})" class="btn btn-sm btn-alt-danger js-bs-tooltip-enabled"
              ...
              <i class="fa fa-trash-alt"></i>
					...
					dataTable.row.add([`${row.FULLNAME}`, `${row.AMOUNT}`, `${row.REMARKS}`, status, btn]).draw();
				});
